...
mpathUtil.getChildPath = (doc, pathSeparator, emptyPathValue) => {
  let basePath = doc.path === emptyPathValue ? pathSeparator : doc.path;
...
function mpathPlugin(schema, options) {
  ...
  schema.pre('save', function preSave(next) {
    ...
    if (this.parent) {
      this.collection
        ...
        .then(parentDoc => {
          let newPath = mpathUtil.getChildPath(parentDoc, pathSeparator, emptyPathValue);
          self.path = newPath;
  ...
  schema.pre('remove', function preRemove(next) {
    ...
    if ('DELETE' === onDelete) {
      const deleteConditions = {
        path: { $regex: '^' + mpathUtil.getChildPath(this, pathSeparator, emptyPathValue) },
      };
  ...
  schema.statics.getChildrenTree = function getChildrenTree(args) {
    ...
    // filters
    if (rootDoc) {
      filters.path = { $regex: '^' + mpathUtil.getChildPath(rootDoc, pathSeparator, emptyPathValue) };
    }
