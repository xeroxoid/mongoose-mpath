...
mpathUtil.getChildPath = (doc, pathSeparator, emptyPathValue) => {
  let basePath = this.path === emptyPathValue ? pathSeparator : this.path;
  return basePath + this._id.toString() + pathSeparator;
};
...
function mpathPlugin(schema, options) {
  ...
  schema.pre('save', function preSave(next) {
    ...
    const updateChildPaths = (pathToReplace, replacementPath) => {
      ...
      const onStreamData = (childDoc, done) => {
        ...
        self.collection.updateMany({ _id: childDoc._id }, { $set: { path: newChildPath } }).then(() => done());
      };
    ...
    if (this.parent) {
      this.collection
        ...
        .then(parentDoc => {
          ...
          if (parentDoc.path === emptyPathValue) {
            newPath = `${pathSeparator}${parentDoc._id.toString()}${pathSeparator}`;
          } else {
            newPath = `${parentDoc.path}${parentDoc._id.toString()}${pathSeparator}`;
          }
  ...
  schema.pre('remove', function preRemove(next) {
    ...
    if ('DELETE' === onDelete) {
      ...
      if (this.path === emptyPathValue) {
        deleteConditions = {
          path: { $regex: '^' + pathSeparatorRegex + this._id.toString() + pathSeparatorRegex },
        };
      } else {
        deleteConditions = {
          path: { $regex: '^' + this.path + pathSeparatorRegex + this._id.toString() + pathSeparatorRegex },
        };
      }
  ...
  schema.virtual('childrenPath').get(function virtualPropChildrenPath() {
    let basePath = this.path === emptyPathValue ? pathSeparator : this.path;
    return basePath + this._id.toString() + pathSeparator;
  });
  ...
  schema.methods.getAncestors = function getAncestors(conditions, fields, options) {
    conditions = conditions || {};
    fields = fields || null;
    options = options || {};
  ...
  schema.statics.getChildrenTree = function getChildrenTree(args) {
    ...
    // filters
    if (rootDoc) {
      if (rootDoc.path === emptyPathValue) {
        filters.path = { $regex: '^' + pathSeparator + rootDoc._id.toString() + pathSeparator };
      } else {
        filters.path = { $regex: '^' + rootDoc.path + rootDoc._id.toString() + pathSeparator };
      }
    }
